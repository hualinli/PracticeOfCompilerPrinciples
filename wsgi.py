...
def lab1():
    if request.method == 'POST':
        ...
        # 渲染模板,显示代码和结果
        return render_template('lab1.html', code=read_code(os.path.join(app.config['UPLOAD_DIR'], code_file.filename)), tokens=tokens, symbols=symbols)
...
def lab2():
    ...
    if request.method == 'POST':
        ...
        codes = read_code(os.path.join(app.config['UPLOAD_DIR'], code_file.filename))
